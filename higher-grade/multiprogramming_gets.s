...
CUSTOM_GETC_RESULT: 	.asciiz "\nCustom getc ==> X\n"
    ...
    # Labels for use in determining the start-address of the buffer-array.
SIZE_8_STRING: .space 8
SIZE_12_STRING: .space 12
SIZE_20_STRING: .space 20
...
TODO_1: # Save $a0, $a1, $s0 to the context.    DONE
    sw $a0,  8($k1)  # $a0
	sw $a1,  12($k1) # $a1
	sw $s0,  16($k1) # $s0  	
...
TODO_2: # Restore $a0, $a1, $s0 from the context. (DONE)
...
TODO_3: # Jump to label __system_call_getc for system call code 12.     (DONE)
...
TODO_4: # Put the jid of the caller in register $a0.    (DONE)
	lw $a0, __running
